...
def test_series_legacy():
    ...
    assert (test == ss1.as_pandas() / ss1.as_pandas()).all()
    assert test.name in (None, '')
    assert test.units == 'dimensionless'
    ...
    test = y // ss4
    expected = y.value // ss4.to_numpy()
    assert (test == expected).all()
    assert test.name == 'length'
    assert test.units == y.units

    test = f // ss4
    expected = f.values // ss4.to_numpy()
    assert (test == expected).all()
    assert test.name == 'length'
    assert test.units == f.units
    ...
    test = m // ss3
    expected = m.values // ss3.to_numpy()
    assert (test - expected < 1e-6).all()
    assert test.name == 'length'
    assert test.units == m.units
    ...
    test = y % ss4
    assert (test == y.value % ss4.to_numpy()).all()
    assert test.name == y.name
    assert test.units == y.units

    test = f % ss4
    assert (test == f.values % ss4.to_numpy()).all()
    ...
    test = m % ss3
    expected = (m.values % ss3.to_numpy())
    assert (np.round(np.array(test), 4) == np.round(expected, 4)).all()
